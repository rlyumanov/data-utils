...
# Предполагаемая структура вашего пакета
...
class TestS3Uploader:
    ...
    def test_init(self):
        ...
        with patch('data_utils.s3.s3.boto3.client') as mock_boto_client:
            ...
            assert uploader.debug == False # По умолчанию False
    ...
    def test_files_are_equal_true(self, s3_uploader, temp_file):
        ...
        mock_client.head_object.return_value = {
            ...
            'ETag': '"simplemd5hash123456"', # Простой MD5 без мультипарт
            'LastModified': datetime.fromtimestamp(os.path.getmtime(temp_file))
        }

        # _calculate_local_file_hash (для соответствия ETag)
        with patch.object(uploader, '_calculate_local_file_hash', return_value='simplemd5hash123456'):
            result = uploader._files_are_equal(temp_file, 'test-bucket', 'test-key')
    ...
    def test_files_are_equal_false_mtime_large_file(self, s3_uploader, temp_file):
        ...
        with patch('os.stat') as mock_stat:
            ...
            result = uploader._files_are_equal(temp_file, 'test-bucket', 'large-test-key')
    ...
    def test_upload_file_success(self, s3_uploader, temp_file):
        ...
        mock_client.upload_file.assert_called_once_with(temp_file, 'dest-bucket', 'uploaded/path/file.txt')
    ...
    def test_upload_file_skip_if_exists_true(self, s3_uploader, temp_file):
        ...
        with patch.object(uploader, '_files_are_equal', return_value=True):
            ...
            with patch.object(uploader, '_get_s3_url', return_value='https://test.url/bucket/key'):
                uploader.upload_file(temp_file, 'key', bucket_name='bucket', skip_if_exists=True)
    ...
    def test_upload_directory_success(self, s3_uploader):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(uploader, 'upload_file') as mock_upload_file:
                uploader.upload_directory(temp_dir, s3_prefix='uploads/', skip_if_exists=True)

                # Проверяем, что upload_file был вызван дважды с правильными аргументами
                expected_calls = [
                    call(file1_path, 'uploads/file1.txt', 'my-bucket', True),
                    call(file2_path, 'uploads/subdir/file2.txt', 'my-bucket', True)
                ]
                mock_upload_file.assert_has_calls(expected_calls, any_order=False)
    ...
    def test_download_file_success(self, s3_uploader):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_client.download_file.assert_called_once_with('source-bucket', 'source/key/file.txt', local_path)
    ...
    def test_list_files_success(self, s3_uploader):
        ...
        mock_client.list_objects_v2.assert_called_once_with(Bucket='list-bucket', Prefix='folder/')
    ...
    def test_delete_file_success(self, s3_uploader):
        ...
        mock_client.delete_object.assert_called_once_with(Bucket='delete-bucket', Key='to/delete/key.txt')
    ...
    def test_create_bucket_success(self, s3_uploader):
        ...
        # Сначала проверим случай, когда бакет уже существует (BucketAlreadyOwnedByYou)
        from botocore.exceptions import ClientError
        error_response_owned = {'Error': {'Code': 'BucketAlreadyOwnedByYou', 'Message': 'Already owned'}}
        mock_client.create_bucket.side_effect = ClientError(error_response_owned, 'CreateBucket')
